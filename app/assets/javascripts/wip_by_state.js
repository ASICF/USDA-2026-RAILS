$(document).on("turbolinks:load", function () {
  ...
  $f = $("#wip_by_state_report.ui.form");
  if ($f.length) {
    // Initialize the Form module
    var form = new Form($f);

    // Compare date ranges
    $.fn.form.settings.rules.dateGreaterThan = function (
      inputValue,
      validationValue
    ) {
    ...
    $(".month-dropdown").dropdown("setting", "onChange", function (value) {
      ...
      if (value == "ALL") {
        month_start = moment("January 1, " + year, "MMMM Do, YYYY").format(
          "MMMM Do, YYYY"
        );
        month_end = moment("December 31, " + year, "MMMM Do, YYYY").format(
          "MMMM Do, YYYY"
        );
      } else {
