class EoTrackerController < ApplicationController
  def index
    @uploads = []
    ...
    Upload.includes(:footprints).where(upload_type: "Footprint", footprints: {flight_date_time: nil, associated: true}).order(id: :ASC).each do |upload|
      ...
      ReportHistory.create(name: "EO Tracker", user: @current_user)
  ...
  def generate_shapefile
    ...
    if params[:strip_frames].blank?
      ...
      response = Footprint.eo_shapefile_export upload, footprints, current_user
