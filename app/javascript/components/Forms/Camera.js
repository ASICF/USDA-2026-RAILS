...
export default function Camera({ record, total_flown, companies, token }) {
  ...
  console.error("Cameras", { record, total_flown, companies, token, errors });
  ...
  const onSubmit = (data) => {
    ...
    for (const property in data) {
      obj[`camera[${property}]`] = data[property];
    }

    if (record.id) {
      axios
        .patch(`/cameras/${record.id}`, {
          camera: data,
  ...
  return (
    <div>
      <Breadcrumbs>
        ...
        <Breadcrumb.Divider />
        <Breadcrumb.Section>Cameras</Breadcrumb.Section>
        {record.id ? (
          <Fragment>
            <Breadcrumb.Divider />
      ...
      {!newRecord() && (
        <MessageBox
          title={"Notice"}
          message={`If a camera has an associated Footprint it cannot be deleted. Selected Camera has ${total_flown} Footprints.`}
        />
      )}
      {renderMessage()}
      {renderDestroyPrompt()}
