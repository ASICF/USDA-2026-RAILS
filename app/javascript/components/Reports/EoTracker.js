...
export default function EoTracker({ uploads, token }) {
  ...
  console.log("EO Tracker", { data, upload });
  ...
  return (
    <div>
      <Breadcrumbs>
        ...
        <Breadcrumb.Divider />
        <Breadcrumb.Section active>EO Tracker</Breadcrumb.Section>
      </Breadcrumbs>
        ...
        <Table celled selectable sortable textAlign="center">
          <Table.Header>
            ...
            <Table.Row>
              <Table.HeaderCell
                sorted={
                  column === "footprints_that_need_eos" ? direction : null
                }
                onClick={() =>
                  dispatch({
                    type: "CHANGE_SORT",
                    column: "footprints_that_need_eos",
                  })
                }
              >
                Missing EOs
              ...
              <Table.HeaderCell
                sorted={column === "footprints_with_eos" ? direction : null}
                onClick={() =>
                  dispatch({
                    type: "CHANGE_SORT",
                    column: "footprints_with_eos",
                  })
                }
              >
                Have EOs
  ...
  function RenderUpload({ upload, setUpload, token }) {
    ...
    useEffect(() => {
      axios
        .post("/query_eo_tracker", {
          upload_id: upload.upload_id,
          authenticity_token: token,
        })
        .then((res) => {
          console.log("query_eo_tracker", res);
    ...
    const exportShapefile = () => {
      ...
      axios
        ...
        .then(({ data }) => {
          ...
          if (data.state) {
            window.open(`/eo_tracker/download/${data.history_id}`, "_blank");
          } else {
            console.error(data);
            setMessage({
              status: "Error",
