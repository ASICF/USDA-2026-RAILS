...
export default function OtherSl({
  results
}) {
  console.log("OtherSl", {
    results
  ...
  const renderRecords = () => {
    ...
    var totals = {
      county_name: results[0].county_name,
      total_flown: 0,
      acres: 0,
      asi_accepted: 0,
      asi_rejected: 0,
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      if (totals.county_name != record.county_name) {
        list.push(
          <Table.Row
            key={`summary_${i}`}
            style={{ background: "#efefef", fontWeight: "bold" }}
          >
            <Table.Cell colSpan="3">County Totals</Table.Cell>
            <Table.Cell>{totals.total_flown}</Table.Cell>
            <Table.Cell>{totals.acres.toFixed(2)}</Table.Cell>
      ...
      list.push(
        <Table.Row key={i}>
          ...
          <Table.Cell>{record.acres}</Table.Cell>
      ...
      grand_total.acres += record.acres;
      ...
      if (i == results.length - 1) {
        ...
        list.push(
          <Table.Row
            ...
            style={{ background: "#efefef", fontWeight: "bold" }}
          >
            <Table.Cell colSpan="3">Grand Total</Table.Cell>
            <Table.Cell>{grand_total.total_flown}</Table.Cell>
            <Table.Cell>{grand_total.acres.toFixed(2)}</Table.Cell>
  ...
  return (
    <Fragment>
      ...
      <Table celled textAlign="center">
        <Table.Header>
          <Table.Row>
            <Table.HeaderCell>County Name</Table.HeaderCell>
            <Table.HeaderCell>Company Name</Table.HeaderCell>
            <Table.HeaderCell>Camera Name</Table.HeaderCell>
            <Table.HeaderCell>Total Flown</Table.HeaderCell>
            <Table.HeaderCell>Total Acres Flown</Table.HeaderCell>
