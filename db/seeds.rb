...
#         if Rails.application.secrets.active_sl_states.include?(record.attributes["STUSPS"]) && State.find_by(abv: record.attributes["STUSPS"]).nil?
...
#     file.each do |record|
#         p "++++++++++++"
#         puts "Record number #{record.attributes["TZID"]}:"
...
#     contract_award = ContractAward.find_by(state: state)

#     # update the easements with the state
#     state.easements.update(contract_award: contract_award)
#     state.tiles.update(contract_award: contract_award)

#     # update the flown tiles
#     state.tiles.flown.each { |tile| tile.set_contract_rate }
