    ...
    task pr_vi: :environment do
      p "Starting Puerto Rico and Virgin Island import"
        ...
        RGeo::Shapefile::Reader.open("#{Rails.application.secrets.pr_vi_state_path}") do |file|
            ...
            p "--------------------"
            file.each do |record|
                p "- State: #{record.attributes["name"].strip}"
                State.create(
                    fips: record.attributes["fips"].strip,
                    abv: record.attributes["abv"].strip,
        ...
        # Import the Counties
        RGeo::Shapefile::Reader.open("#{Rails.application.secrets.pr_vi_county_path}", encoding: 'ISO-8859-1') do |file|
            p " "
            p "Counties"
            p "--------------------"
            ...
            file.each do |record|
                p "- County: #{record.attributes["NAME"].strip}"
