define([
    ...
    "dojo/_base/url",
    "dojo/_base/lang"
],  function (declare, Graphic, GraphicsLayer, InfoTemplate, graphicsUtils, Color, Font, TextSymbol, SimpleMarkerSymbol,
    ...
    return declare([GraphicsLayer], {
        ...
        constructor: function (options) {
            ...
            // Default popup
            if (options.infoTemplate !== false && options.title) {
                this.setInfoTemplate(options.infoTemplate || new InfoTemplate(options.title, options.template));
            }
            // Renderer
            if (options.renderer) {
            ...
            if (options.label) {
                ...
                this.label_layer = new GraphicsLayer();

                // Add a min scale if in params
                if (options.label.minScale) {
                    this.label_layer.setMinScale(options.label.minScale);
                }

                // Add a max scale if in params
                if (options.label.maxScale) {
                    this.label_layer.setMaxScale(options.label.maxScale);
                }

                // Set the label field to reference
                this.label_field = options.label.field;

                // Create the Font
                var font = new Font();
                font.setSize(options.label.font_size || 14);
                font.setFamily("lato");
                font.setWeight(Font.WEIGHT_BOLD);

                // Create the Text Symbol
                this.label_symbol = new TextSymbol();
                this.label_symbol.setFont(font);
                this.label_symbol.setOffset(options.label.xoffset || 0, options.label.yoffset || 0);
                this.label_symbol.setHaloColor(new Color([240, 240, 240]));
                this.label_symbol.setColor(options.label.color);
                this.label_symbol.setHaloSize(1);
                this.label_symbol.setHorizontalAlignment(options.label.horizontal || "center");
                this.label_symbol.setVerticalAlignment(options.label.vertical || "middle");
            ...
            // Manage drawing
            this._maxDraw = options.maxdraw || 1000000;  // default to 1000 graphics
            this._drawCount = 0;
        ...
        _createGraphic: function (arcgisJson) {
            ...
            if (this.renderer && this.renderer.symbol) {
                ...
                if (this.label_layer) {
                    ...
                    // Copy the TextSymbol so that we can add a custom label
                    var text_symbol = new TextSymbol(this.label_symbol.toJson());
                    text_symbol.setText(graphic.attributes[self.label_field]);
    
                    text_graphic = new Graphic(
                        graphic.geometry,
                        text_symbol,
                        {}
                    )
    
                    this.label_layer.add(text_graphic);
                }
